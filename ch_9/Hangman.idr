...
notSingletonList : ValidInput (x :: (y :: ys)) -> Void
notSingletonList (Letter _) impossible

isValidInput : (cs : List Char) -> Dec (ValidInput cs)
isValidInput [] = No emptyNotValid
isValidInput [x] = Yes (Letter x)
isValidInput (x :: (y :: ys)) = No notSingletonList
...
main : IO ()
main = do
  result <- game {guesses=3} (MkWordState "Test" ['T', 'E', 'S'])
  case result of
       Lost (MkWordState word missing) => putStrLn ("You lose. The work was " ++ word)
